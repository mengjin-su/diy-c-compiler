...
static int caseValCheck(int *val_list, int length, int i, int val, int default_idx);

void Pcoder :: run7(oprNode_t *op)
{
	int depth = DEPTH();
	int code  = OPR_TYPE(op);
	int true_lbl, false_lbl, end_lbl, loop_lbl;
...
void Pcoder :: switchListProc(Item *ip, node *list)
{
	...
	int *lbl_list = new int[length];	// hold case labels
	int *val_list = new int[length];	// hold case values
	int  end_lbl  = getLbl();			// end label
	int  default_idx = -1;				// default case label index
	int  expr_size = ip->acceSize();

	pnp = new Pnode(SWITCH);	
	...
	for (int i = 0; i < length; i++)
	{
		...
		{
			default_idx = i;
			length = i + 1;		// skip following 'case' if any
			continue;
		}
		...
		if ( DEPTH() == (depth+1) )
		{
			...
			else if ( caseValCheck(val_list, length, i, cip->val.i, default_idx) )
...
static int caseValCheck(int *val_list, int length, int i, int val, int default_idx)
{
	for (int j = 0; j < i; j++)
	{
		if ( j != default_idx && val_list[j] == val )
			return -1;
	}
	return 0;
}
