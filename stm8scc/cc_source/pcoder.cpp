...
void Pcoder :: run(node *np)
{
	...
	switch ( np->type )
	{
		...
		case NODE_STR:	// string Node
			ip = idItem(constGroup->add(np->str.str));
			ip->attr = newAttr(CHAR);
			ip->type = IMMD_ITEM;
			ip->attr->dataBank = CONST;
			PUSH(ip);
			break;
		...
		case NODE_ID:	// ID node
			...
			{
				ip = (dp->attr->dimVect)? immdItem(dp->nameStr(), dp->attr):
			...
			if ( op->nops > 0 && op->op[0] )
			{
				...
				if ( data_def )
				{
					if ( np->id.init )	// data initialization ...
					{
						...
						{	// function non-static local data ...
							int depth = DEPTH();
							run(np->id.init);
							if ( DEPTH() == (depth+1) )
							{
					...
					else if ( dp->attr->dimVect )	// array defined
					{
						...
						for (int i = 0; i < v[0]; i++) {
							if ( v[i+1] <= 0 ) {
								errPrint("unknown array dimension!", dname);
								break;
							}
						}
					}
...
/*
		case INTERRUPT:
			np = (node*)ap->atAddr;	// get vector value (node)
			if ( !(np->type == NODE_CON && (np->con.value & ~0x7c) == 0x8000) )
			{
				errPrint("ISR vector error!");
				break;
			}

			sprintf(buf, "%s_0x%lX", INTERRUPT_ISR_NAME, np->con.value);

			if ( funcLink->search(buf) )
				errPrint("ISR redefined!");
			else
			{
				Fnode *fp = new Fnode(buf, INTERRUPT);
				fp->attr = cloneAttr(ap);		// copy it's (return) type.
