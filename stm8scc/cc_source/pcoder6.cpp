	...
	switch ( code )
	{
			...
			if ( DEPTH() == (depth+2) )
			{
				...
				if ( ip0->type == CON_ITEM && ip1->type == CON_ITEM )
				{
					int n = (code == '*')?     ip0->val.i *  ip1->val.i:
							...
							(code == '%')? 	   ip0->val.i %  ip1->val.i:
							(code == LEFT_OP)? ip0->val.i << ip1->val.i:
											   ip0->val.i >> ip1->val.i;
					PUSH(intItem(n));
					return;
				}
				...
				if ( ip0->isMonoVal() && ip1->isMonoVal() )
				{
					...
					if ( code == LEFT_OP )
					{
						...
						tmp->attr->isUnsigned = !ip0->acceSign();
					}
					else
		...
		case '<': 	case '>':
			...
			if ( DEPTH() == (depth+2) )
			{
				...
				{
					int n = 0;
					if ( code == EQ_OP && ip0->val.i == ip1->val.i ) n = 1;
					if ( code == NE_OP && ip0->val.i != ip1->val.i ) n = 1;
					if ( code == LE_OP && ip0->val.i <= ip1->val.i ) n = 1;
					if ( code == GE_OP && ip0->val.i >= ip1->val.i ) n = 1;
					if ( code == '<'   && ip0->val.i <  ip1->val.i ) n = 1;
					if ( code == '>'   && ip0->val.i >  ip1->val.i ) n = 1;
					PUSH(intItem(n));
				...
				PUT_LBL(true_lbl);
				pnp = new Pnode(P_INC);	addPnode (&mainPcode, pnp);
				pnp->items[0] = tmp;
