...
void expandPtr(attrib *aptr, int type)
{
	...
	else if ( aptr )
	{
		int l1 = aptr->ptrVect[0];
		int *ptr = MALLOC(sizeof(int)*(l1 + 1 + 1));
		memcpy(ptr, aptr->ptrVect, sizeof(int)*(l1+1));
		ptr[0]    = l1 + 1;
		ptr[l1+1] = type;
		free(aptr->ptrVect);
		aptr->ptrVect = ptr;
...
{
	attrib *ap = (attrib *)MALLOC (sizeof (attrib));
	ap->type = atype;
	return ap;
}
/*
...
attrib *cloneAttr(attrib *ap)
{
	...
	*a = *ap;
	...
	a->indxAttr= cloneAttr(ap->indxAttr);
...
int cmpAttr(attrib *ap1, attrib *ap2)
{
	...
	if ( (ap1->parList && !ap2->parList) ||
		 (ap2->parList && !ap1->parList) )
		return -1;
...
{
    node *p = newNode(NODE_CON);
    p->con.attr = newAttr(type);
//  p->con.attr->isUnsigned = 1;
    p->con.value= v;
    return p;
}
...
node *strNode (char *str)
{
    node *p = newNode(NODE_STR);
    p->str.str = dupStr(str);
    return p;
}
