...
Dnode *Fnode ::	getData(Dnode *dnp, FDATA_t type, int *seq, int *offset)
{
...
Dnode *Fnode ::	getData(Dlink *dlp, FDATA_t type, int *seq, int *offset)
{
	for(int offs = *offset; dlp; dlp = dlp->next)
	{
		Dnode *d;
		*offset = offs;

		d = getData(dlp->dlist, type, seq, offset);
		if ( d ) return d;

		d = getData(dlp->child, type, seq, offset);
		if ( d ) return d;
	}
	return NULL;
}

Dnode *Fnode ::	getData(FDATA_t type, int *seq, int *offset)
{
	*offset = 0;	// offset start at '0'

	// skip parameter link
	return getData(dlink->child, type, seq, offset);
}
