...
void Item :: updateAttr (attrib *ap)
{
	delAttr(attr);
	attr = ap;
...
bool Item :: isWritable(void)
{
	switch ( type )
	{
		...
		case INDIR_ITEM:
		case DIR_ITEM:
			if ( attr->ptrVect[1] == CONST ) return false;
//			if ( attr->type == VOID )	 return false;
		default:
			break;
	}
...
bool Item :: isOperable (void)
{
	...
	if ( isMonoVal() ||
		 type == STR_ITEM || type == IMMD_ITEM || type == LBL_ITEM )
		return true;

	if ( attr->dimVect && !attr->ptrVect )
		return false;
...
{
	if ( type == CON_ITEM || type == IMMD_ITEM ) return true;
	if ( attr == NULL )	return false;
	if ( attr->dimVect || attr->isFptr ) return false;
	...
	switch ( type )
	{
		...
		case PID_ITEM:
		case DIR_ITEM:
			if ( ptrWeight(attr) != 1 || attr->type == VOID )
				return false;
			break;
	...
	return (attr->type == CHAR  || attr->type == INT ||
...
bool Item :: isAccePtr (void)
{
	switch ( type )
	{
		...
		case TEMP_ITEM:
		case ACC_ITEM:
			return attr->ptrVect? true: false;
...
Item *accItem(int size, bool is_signed)
{
	attrib *attr = newAttr((size == 1)? CHAR:
						   (size == 2)? INT:
						   (size == 3)? SHORT: LONG);
	attr->isUnsigned = is_signed? 0: 1;
	return accItem(attr);					   
}
...
bool same(Item *ip0, Item *ip1)
{
	if ( ip0 == NULL && ip1 == NULL ) return true;
	if ( ip0 == NULL || ip1 == NULL ) return false;
	if ( ip0->type != ip1->type ) return false;
	if ( cmpAttr(ip0->attr, ip1->attr) ) return false;
	if ( ip0->bias != ip1->bias ) return false;
...
bool equivalent(Item *ip0, Item *ip1)
{
	if ( !(ip0 && ip1) )
		return false;
	...
	return (ip0->type == ip1->type &&
			ip0->acceSize() == ip1->acceSize() &&
			(ip0->type == ACC_ITEM || sameTemp(ip0, ip1)));
}

bool isDirectReg(Item *ip)
{
	int size = ip->acceSize();
	return (size == 1 && ip->type == DIR_ITEM && ip->val.i >= 0x4000 && ip->val.i < 0x5800);
}