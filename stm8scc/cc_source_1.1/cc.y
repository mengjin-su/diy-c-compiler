...
static void bitFieldOffsetAssign(node *p);
...
%token FOR GOTO IF REGISTER RETURN SIZEOF STATIC SWITCH 
...
func_definition
	...
	| INTERRUPT identifier
	  '(' conditional_expr ')'	{ CLR_SRC(); }
	  compound_statement		{ node *np = oprNode(FUNC_HEAD, 1, $2);
	  							  $$ = oprNode(FUNC_DECL, 2, np, $7); 
...
static void bitFieldOffsetAssign(node *p)
{
	if ( p && p->type == NODE_LIST )
	{
		int i, offset = 0;
		for (i = 0; i < p->list.nops; i++)
		{
			node *np1 = LIST_NODE(p, i);
			if ( np1->type == NODE_OPR && np1->opr.oper == DATA_DECL && np1->opr.nops == 1 )
			{
				attrib *ap = np1->opr.attr;
				if ( ap && ap->bitField )
				{
					if ( (offset + ap->bitField) > 8 ) offset = 0;

					ap->bitField |= (offset << 8);			// set offset for the field
					offset 		 += (ap->bitField & 0xff);	// adjust offset for next...
					continue;
				}
			}
			offset = 0;
