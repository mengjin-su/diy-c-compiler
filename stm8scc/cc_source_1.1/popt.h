...
class Optimizer {
	...
	private:
		...
		void replaceTemp(Pnode *pnp, int tmp_old, int tmp_new);
		bool reduceTempSize(Pnode *pnp, Item *tmp, int size);
		bool accReferenced(Pnode *pnp);
		bool replaceIndir(Pnode *pnp, Item *ip, int temp_index);
		void swap(Pnode *pnp1, Pnode *pnp2);
