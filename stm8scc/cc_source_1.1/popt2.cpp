...
{	/*
	...
	*/
	...
	int type  = pnp->type;
	int rev_type, n;
	if ( convertJump(type, &rev_type, &n) && p1 && p2 )
	{
...
{	/*
	...
	*/
	...
	if ( pnp->type == '=' && ITYPE(ip0, TEMP_ITEM) && ITYPE(ip1, CON_ITEM) )	//v1.15
	{
		Pnode *p1 = indirReferenced(pnp->next, ip0, &i);
		if ( p1 && ITYPE(p1->items[i], INDIR_ITEM) )
		{
			attrib *attr = cloneAttr(p1->items[i]->attr);
			reducePtr(attr);
			attr->dataBank = ip1->attr->dataBank;
...
bool Optimizer :: case10(Pnode *pnp)
{
	int type = pnp->type;
	int swap_type;
	if ( (type == P_JEQ || type == P_JNE) &&
		 ITYPE(ip1, CON_ITEM) && ip1->val.i == 0 )
	{
		pnp->type = (type == P_JEQ)? P_JZ: P_JNZ;
		pnp->updateItem(1, ip2);
		pnp->items[2] = NULL;
		return true;
	}
	if ( swapJump(type, &swap_type) &&
