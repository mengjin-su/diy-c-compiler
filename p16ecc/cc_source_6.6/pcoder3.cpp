...
#define CLONE_ST_UN_NEWDATA(a1,a2)	if ( a1->type == a2->type && a1->typeName && a2->typeName && \
									     strcmp(a1->typeName, a2->typeName) == 0 &&		\
									     a1->newData == NULL 					  )		\
										a1->newData = cloneNode((node*)a2->newData);
			...
			if ( DEPTH() == (depth+1) )
			{
				...
				switch ( i0_type = ip0->type )
				{
					...
					case INDIR_ITEM: case PID_ITEM:	case DIR_ITEM:
						...
						data_bank = (i0_type == IMMD_ITEM)? attr->dataBank:
															reducePtr(attr);
