...
int main(int argc, char *argv[], char *env[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
		...
		if ( !start_parsing )
		{
			...
			addSymbol(&libList, new Symbol((char*)"crt1"));
			start_parsing++;
		}
	...
	if ( start_parsing )
	{
		...
		p16link.scanInclusion();			// remove unused segments
		p16link.assignSegmentsAddress();	// allocate memories

		if ( p16link.errorCount == 0 )
		{
			std::string hex_output = outputFile;	hex_output += ".hex";
			std::string map_output = outputFile;	map_output += ".map";
			p16link.outputHex((char*)hex_output.c_str());
			p16link.outputMap((char*)map_output.c_str());
...
static char *getLibFile(char *libfile)
{
	...
	std::string as_cmd = useBSR6? "as16e -X ": "as16e ";
