...
void P16link :: assignFuncLocalData(Symbol **symlist)
{
	...
	int group_list[fcallMgr.funcCount];
	...
	for (bool done = false; !done;)
	{
		...
		{
			FuncAttr *fp = fcallMgr.getFunc(i);
			if ( !func_list[i] && fp->dataSize > 0 )
			{
				if ( fcallMgr.joinInGroup(group_list, length, i) )
				{
					group_list[length++] = i;
					func_list[i] = 1;
					if ( max_size < fp->dataSize ) max_size = fp->dataSize;
				}
			}
		}
		...
		if ( max_size && dataMem->getSpace(0, &addr, max_size) )
		{
			...
			{
				FuncAttr *fp = fcallMgr.getFunc(group_list[i]);
