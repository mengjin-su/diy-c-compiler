...
#define SET_C_FLG	STATUS |=  1
#define CLR_C_FLG	STATUS &= ~1
#define TST_C_FLG	(STATUS & 1)
...
void _signedCmpAndZ(void)
{
	WREG = STATUS;			// WREG := STATUS
	_BTFSC(INDF1, 7);
	WREG ^= 1;				// WREG[0] = Sx ^ Sy ^ C
	asm("addfsr FSR1, 1");
	_BTFSC(WREG, 2);		// skip if Z = 0 (~Z = 1)
	WREG &= ~1;				// asm16e->code(_BCF, aWREG, 0);
}

void _signedCmpOrZ(void)
{
	WREG = STATUS;			// WREG := STATUS
	_BTFSC(INDF1, 7);
	WREG ^= 1;				// WREG[0] = Sx ^ Sy ^ C
	asm("addfsr FSR1, 1");
	_BTFSC(WREG, 2);		// skip if Z = 0
	WREG |= 1;				// asm16e->code(_BSF, aWREG, 0);
}
