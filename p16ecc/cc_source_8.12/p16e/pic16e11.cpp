...
void PIC16E :: andorxor(int code, Item *ip0, Item *ip1, Item *ip2)
{
	...
	if ( useFSR(ip0, ip1) || 
		 useFSR(ip1, ip2) ||
		 (CONST_ID_ITEM(ip1) && CONST_ID_ITEM(ip2))	)
	{
		mov(ip0, ip1);
		andorxor(logicAssign(code), ip0, ip2);
		return;
...
void PIC16E :: andorxor(int code, Item *ip0, Item *ip1, int n)
{
	...
	{
		int  num = n & 0xff;
		bool load_constant = false;
		
		switch ( num )
		{
			case 0x00:	// constant byte = 0x00
				if ( i < size1 && code != '&' )
					fetch(ip1, i, indf1);
				else
					load_constant = true;
				break;
			case 0xff:	// constant byte = 0xFF
				if ( i < size1 && code == '&' )
					fetch(ip1, i, indf1);
				else if ( i < size1 && code == '^' )
