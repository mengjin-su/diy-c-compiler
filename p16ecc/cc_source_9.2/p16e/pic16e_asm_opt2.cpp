	...
	if ( bufferDepth() >= 2 )
	{
		...
		if ( bufferDepth() >= 3 )
		{
			...
			if ( p1->isInst(_MOVWF) && p2->isInst(_MOVF, _W_) &&
				 sameOperand(p1, p2) && isUpdate_WandZ(p0) )
		 	{
				 removeLine(idx2);	 return 1;
			}
			if ( p0->isInst(_MOVWF) && p1->isInst(_MOVF, _W_) &&
				 sameOperand(p0, p1) && isUpdate_WandZ(p2, false) )
		 	{
				 removeLine(idx1);	 return 1;
			}
		}
	}
	return 0;
...
int P16E_asmOPT :: case8(void)
{
	if ( bufferDepth() >= 2 )
	{
		...
		if ( p0->isInst1(_MOVWI, "--INDF1") && p1->isInst1(_MOVIW, "INDF1++") )
		{
			p0->inst = _IORLW;
			p0->opr1 = "0";
			removeLine(idx1);
