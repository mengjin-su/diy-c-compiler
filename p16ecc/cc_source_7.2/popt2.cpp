...
bool Optimizer :: group2(Pnode *pnp)
{
	...
	{
		pnp->updateItem(0, p1->items[0]);								// {'+', [z, x, y]}
		p1->items[0] = NULL;
		delete p1;
		return true;
	}
	...
	{
		swap(p1, p2);	// swap line p1 and p2.
		return true;
	}

	if ( (ptype == '+' || ptype == '-') &&
		 ip0->type == TEMP_ITEM && ip2->type == CON_ITEM )					// {'+', [t, x, N]}
	{																		// {'+', [y, t, M]}
		if ( p1 && (p1->type == '+' || p1->type == '-') &&
			 same(ip0, p1->items[1]) && p1->items[2]->type == CON_ITEM &&
			 (related(p1->items[0], ip0) || unusedTmp(p1->next, ip0->val.i)) )
		{
			if ( ptype == p1->type )										// {'+', [y, x, N+M]}
				ip2->val.i += p1->items[2]->val.i;
			else if ( ip2->val.i >= p1->items[2]->val.i )
				ip2->val.i -= p1->items[2]->val.i;
			else {
				ip2->val.i = p1->items[2]->val.i - ip2->val.i;
				pnp->type  = p1->type;
			}
