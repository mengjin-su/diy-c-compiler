...
void PIC16E :: mulAssign(Item *ip0, Item *ip1)
{
    ...
    char *func = (size0 == 2)? (char*)"_mul16indf":
    			 (size0 == 3)? (char*)"_mul24indf":
    						   (char*)"_mul32indf";
	pushStack(ip1, size0);	// push y into the stack.
	setFSR(ip0, 0);			// load FSR0 with the address of x
	call(func);
	regFSR0->reset();
	regWREG->reset();
}
...
{
	int size0 = ip0->acceSize();
	if ( same(ip0, ip2) && ip0->type != ACC_ITEM )
	{
		mulAssign(ip0, ip1);
		return;
	}
...
void PIC16E :: mul8(Item *ip0, Item *ip1, Item *ip2)
{
	...
	if ( ip0->type != ACC_ITEM || ip0->acceSize() > 2 )
		mov(ip0, acc);
