...
#define COMPOUND_ASSIGN(t)	(t == ADD_ASSIGN || t == SUB_ASSIGN || \
...
bool Optimizer :: group5(Pnode *pnp)
{
	...
	if ( TERMINATE_ASSIGN(ptype) || COMPOUND_ASSIGN(ptype) )	// {'+', [z, x, y]} or {'+=', [x, y]} ...
	{
		...
		{
			if ( size1 > size0 && reduceTmpSize(pnp->last, ip1, size0) )
			{
		...
		{
			if ( size2 > size0 && reduceTmpSize(pnp->last, ip2, size0) )
			{
