...
void PIC16E :: jzjnz(int code, Item *ip0, Item *ip1)
{
	if ( ip0->isBF() )
	{
		...
		if ( size0 == 1 )
		{
			...
			ASM_CODE(inst, acceItem(ip0, 0, indf0), shift);
		}
		else
		{
			...
			if ( size0 < 8 )
			{
				...
				ASM_CODE(_ANDWF, acceItem(ip0, 0, indf0), _W_);
			}
			else
				ASM_CODE(_MOVF, acceItem(ip0, 0, indf0), _W_);
...
void PIC16E :: jzjnz_incdec(int code, Item *ip0, Item *ip1, Item *ip2)
{
	...
	if ( ip0->isBF() )
	{
		int size0 = BF_SIZE(ip0->attr);
		int shift = BF_OFFSET(ip0->attr);
		getBF(ip0, shift, size0, TO_STACK);
		incValue(ip0, ip1->val.i);
		ASM_CODE(_MOVIW, "INDF1++");
	}
	else if ( size0 == 1 && abs(n) == 1 )
	{
