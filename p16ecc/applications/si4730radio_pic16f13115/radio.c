...
unsigned int  RADIO_fmFreq = 9990;	// x10KHz -> 99.9MHz
unsigned int  RADIO_amFreq = 550;	// x1KHz  -> 550KHz
void RADIO_dispFreq(void);

void RADIO_init(void)
{
	TRISAbits->b5 = 0;		// PA5 is output
	LATAbits->b5  = 0;		// RST = 0
    TMR0_delayMs(100);
    TRISAbits->b5 = 1;      // RST = 1
    TMR0_delayMs(100);

	KEY_init();				// put PA5 to input
	startFM();
	startFM();
}
