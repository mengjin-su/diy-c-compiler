...
void printfSegments(Segment *slist, bool with_addr)
{
	for (; slist; slist = slist->next)
	{
//		slist->print();
		int type = slist->type();
		int addr = slist->memAddr;
		if ( type == CODE_SEGMENT || type == CONST_SEGMENT ) addr &= 0x7fff;
		...
		switch ( type )
		{
			...
			case FUSE_SEGMENT:
				...
				if ( slist->isABS || with_addr ) printf(" Addr=%d", addr);
				break;
		}
