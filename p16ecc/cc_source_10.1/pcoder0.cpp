...
void Pcoder :: romDataInit(char *dname, attrib *attr, node *init_data, Dnode *dp, int addr)
{
	...
	if ( DEPTH() == (depth+1) )
	{
		...
		if ( FFormat.reform(ip, attr) != 0 ) 
			errPrint("improper form for float!", ip->val.s);
		...
		if ( type == CON_ITEM || type == IMMD_ITEM || type == LBL_ITEM )
		{
			...
			if ( BF_OFFSET(attr) == 0 )
			{
				...
				pnp->items[1] = intItem(size);
				addPnode(&constPcode, pnp);
				lastInitItem = IS_BF_VAR(attr)? ip: NULL;
			}
			else
			{
				if ( lastInitItem &&
				  	 lastInitItem->type == CON_ITEM && type == CON_ITEM )
				{
					lastInitItem->val.i |= ip->val.i << BF_OFFSET(attr);
				}
				delete ip;
			}
		}
