...
node *PreScan :: scan(idNode_t *ip)
{
	...
	{
		node *new_np = scan(cloneNode(dp->np[0]));
...
node *PreScan :: scan(oprNode_t *op)
{
	...
	switch ( op->oper )
	{
		...
		case DEFINE:	// add a macro definition
			if ( op->nops > 1 ) p1 = op->op[1], op->op[1] = NULL;
			if ( op->nops > 2 ) p2 = op->op[2], op->op[2] = NULL;
		...
		case DATA_DECL:
			...
			if ( ap->type == STRUCT || ap->type == UNION )
			{
				...
				{
					if ( (ndp = nlist->search(ap->typeName, ap->type)) == NULL )
						errPrint("struct/union not defined!");

					ins_aap = (ndp == NULL || ndp->attr == NULL);
					aap     = (ndp == NULL || ndp->attr == NULL)? newAttr(ap->type): ndp->attr;
					if ( ins_aap && ap->typeName ) aap->typeName = dupStr(ap->typeName);
				}
			}
			...
			if ( op->nops > 0 && ap && aap )
			{
				...
				for (int i = 0; i < np->list.nops; i++)
				{
					...
					if ( ins_aap && !dnp->opr.attr->ptrVect )
						errPrint("self contained member: ", inp->id.name);
				}
	...
	switch ( op->oper )
	{
		...
		case '>': case '<':	case LE_OP:	case GE_OP:
			...
			{
				if ( op->op[0]->con.attr->type == FLOAT ||
					 op->op[1]->con.attr->type == FLOAT )
				{
					int err;
					node *np = mergeFloatCon(op->op[0], op->op[1], op->oper, &err);
					CLR_COMMENT(op); delNode((node*)op);
					if ( err ) errPrint("illegal float operation!");
					return np;
				}
		...
		case NEG_OF:
			if ( op->op[0]->type == NODE_CON )
			{
				...
				int  n = np->con.value;
				bool is_fp = (np->con.attr->type == FLOAT);

				if ( op->oper == '~' && is_fp )
					errPrint("illegal operation!");
				else if ( op->oper == NEG_OF && is_fp )
					np->con.fvalue = -np->con.fvalue;
				else
				{
					n = (op->oper == '~')? ~n:
					    (op->oper == '!')? (n? 0:1):
...
node *PreScan :: mergeFloatCon(node *np0, node *np1, int code, int *err)
{
	float f0, f1;
	*err = 0;

	f0 = np0->con.fvalue;
	if ( np0->con.attr->type != FLOAT )	f0 = (float)np0->con.value;
	f1 = np1->con.fvalue;
	if ( np1->con.attr->type != FLOAT )	f1 = (float)np1->con.value;
	...
	switch ( code )
	{
		...
		case RIGHT_OP:
					*err = 1;
					break;
		case EQ_OP:	n = (f0 == f1)? 1: 0;	break;
		case NE_OP:	n = (f0 != f1)? 1: 0;	break;
		case AND_OP:n = (f0 && f1)? 1: 0;	break;
		case OR_OP:	n = (f0 || f1)? 1: 0;	break;
		case '>':	n = (f0 >  f1)? 1: 0;	break;
		case '<':	n = (f0 <  f1)? 1: 0;	break;
		case LE_OP:	n = (f0 <= f1)? 1: 0;	break;
		case GE_OP:	n = (f0 >= f1)? 1: 0;	break;
			break;
	}
