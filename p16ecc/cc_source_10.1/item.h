#ifndef _ITEM_H
#define _ITEM_H

#include "common.h"

////////////////////////////////////////////////////////////////
typedef enum {
    STR_ITEM = 1,	// string Item
    CON_ITEM,		// constant Item
    ID_ITEM,		// id Item
    TEMP_ITEM,		// temp Item
    ACC_ITEM,		// ACC Item
    LBL_ITEM,		// label Item
    IMMD_ITEM,		// immediate Item
    INDIR_ITEM,		// indirect address Item
    DIR_ITEM,		// direct address Item
    PTR_ITEM,		// pointer Item
    PID_ITEM,		// pointer variable Item
} ITEM_TYPE;

typedef union {
    int      i;
    char    *s;
    void    *p;
} value;

#define SIGNED(ip)		ip->acceSign()

/////////////////////////////////////////////////////////////////////////////
class Item {

	public:
	    ITEM_TYPE   type;	// item type
	    value       val;
	    int			bias;	// bias for indirect access
	    attrib      *attr;  // attributes
		void     	*home;	// owner of id (ID_ITEM only)
	    Item 		*next;	// linking chain

	public:
		Item(ITEM_TYPE t);
		~Item();

		Item *clone (void);
		void updateAttr (attrib *ap, int data_bank = -1);
		void updatePtr  (int *p);
		void updateName (char *new_name);
		bool isWritable (void);
		bool isOperable (void);
		bool isMonoVal  (void);
		bool isAccePtr  (void);
		bool isBitVal   (void);
		attrib *acceAttr(void);
		int	 stepSize   (void);
		int  acceSize   (void);
		int  acceSign   (void);
		int  storSize	(void);
		bool isBF		(void);
		bool acceFloat	(void);
};

Item *intItem(int val, attrib *ap=NULL);
Item *strItem(char *str);
Item *lblItem(int lbl);
Item *lblItem(char *str);
Item *idItem (char *name, attrib *ap=NULL);
Item *ptrItem(void *p);
Item *immdItem(char *str, attrib *ap);
Item *tmpItem(int index, attrib *ap=NULL);
Item *accItem(attrib *attr);

bool moveMatch(Item *ip0, Item *ip1);
attrib *maxMonoAttr(Item *ip0, Item *ip1, int op);
attrib *maxSizeAttr(Item *ip0, Item *ip1);
bool comparable(int code, Item *ip0, Item *ip1);
bool same(Item *ip0, Item *ip1, bool fsr_check = false);
bool sameTemp(Item *ip0, Item *ip1);
bool overlap(Item *ip0, Item *ip1);
bool related(Item *ip0, Item *ip1);

#define IS_TEMP(ip,idx)			(ip->type == TEMP_ITEM && ip->val.i == idx)

#define SIGNED_RESULT(ip0,ip1)	((ip0->acceSign() && ip1->acceSign()) 	  || \
								 (ip0->acceSign() && ip1->type==CON_ITEM) || \
								 (ip1->acceSign() && ip0->type==CON_ITEM)	 )

#define ANY_FLOAT(ip0,ip1)		((ip0 && ip0->acceFloat()) || (ip1 && ip1->acceFloat()))
#define BOTH_FLOAT(ip0,ip1)		((ip0 && ip0->acceFloat()) && (ip1 && ip1->acceFloat()))

#endif