...
class PIC16E {
	...
	private:
		...
		void leftAssignConst(Item *ip0, int n);
		void rightAssign(Item *ip0, Item *ip1, bool sshift);
		void rightAssignConst(Item *ip0, int n, bool sshift);
		...
		// pic16e12.cpp
		typedef enum {TO_WREG, TO_ACC, TO_STACK} BF_Target;
		void getBF(Item *ip, int left_shift=0, int req_size=8, BF_Target target=TO_WREG);
		void movFromBF(Item *ip0, Item *ip1);
		void movToBF(Item *ip0, Item *ip1);
		void movToBF(Item *ip0, int n);
		void movToBF(Item *ip0, char *s);
		...
		// pic16e14.cpp
		void moveToFP(Item *ip0, Item *ip1);	// float <- ip1
		void moveFromFP(Item *ip0, Item *ip1);	// ip0 <- float
		void pushToStackAsFloat(Item *ip);
		void storeToAccAsFloat(Item *ip);
		void addFP(Item *ip0, Item *ip1);
		void subFP(Item *ip0, Item *ip1);
		void mulFP(Item *ip0, Item *ip1);
		void divFP(Item *ip0, Item *ip1);
		void cmpFP(int code, Item *ip0, Item *ip1, Item *p2);
		void jeqjneFP(int code, Item *ip0, Item *ip1, Item *p2);		
};
