...
void PIC16E :: incValue(Item *ip, int value)
{
	...
	if ( ip->isBF() )
	{
		size0 = BF_SIZE(ip->attr);
		int shift = BF_OFFSET(ip->attr);
		uint8_t mask = ((1 << size0) - 1) << shift;
		int n = (value << shift) & mask;

		if ( (size0 + shift) >= 8 )
		{
			regWREG->load(n);
			ASM_CODE(_ADDWF, acceItem(ip, 0, indf), _F_);
		}
		else if ( n )
		{
			regWREG->load(n);
			if ( size0 > 1 )
			{
				ASM_CODE(_ADDWF, acceItem(ip, 0, indf), _W_);
				ASM_CODE(_XORWF, acceItem(ip, 0, indf), _W_);
				ASM_CODE(_ANDLW, mask);
				regWREG->reset();
			}
			ASM_CODE(_XORWF, acceItem(ip, 0, indf), _F_);
		}
		return;
	}

	if ( value == 1 && !(ip->type == INDIR_ITEM || ip->type == PID_ITEM) )
   	{
		for (int i = 0; i < size0; i++)
		{
			char *ds = acceItem(ip, i, indf);
			if ( i ) ASM_CODE(_BTFSC, aSTATUS, 2);
	...
	{
		int v = value & 0xff;
		if ( (v == 1 || v == 0xff) && !started && (i+1) == size0 ) 	// last byte
		{
			if ( v == 1 )
		...
		else if ( v != 0 || started )								// skip lower '0' value bytes
		{
			regWREG->load(v);
			if ( started )
