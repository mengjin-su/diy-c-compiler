	...
	switch ( code )
	{
		...
		case LEFT_ASSIGN:	case RIGHT_ASSIGN:
			...
			if ( DEPTH() == (depth+2) )
			{
				...
				if ( !(code == MUL_ASSIGN  || code == DIV_ASSIGN) )
				{
					if ( ip0->acceFloat() || ip1->acceFloat() )
					{
						delete ip0;
						delete ip1;
						errPrint("invalid operation for float-point!");
						return;
					}
				}
...
void Pcoder :: ptrBiasing(int code, Item *ip0, Item *ip1)
{
	...
	attrib *ap = pnp->items[0]->attr;
	if ( ap->dimVect )
	{
		if ( ip0->type == IMMD_ITEM )
			insertPtr(ap, ap->dataBank);
