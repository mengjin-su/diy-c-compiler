...
bool Item :: isMonoVal(void)
{
	...
	if ( type == DIR_ITEM || type == PID_ITEM || type == INDIR_ITEM )
	{
		...
		return (attr->type >= CHAR && attr->type <= LONG) || attr->type == FLOAT;
	}
...
bool Item :: isBF(void)
{
	bool attrBF = IS_BF_VAR(attr);

	switch ( type )
	{
		case ID_ITEM:
			return (ptrWeight(attr) == 0 && attrBF);
		case PID_ITEM:
		case INDIR_ITEM:
		case DIR_ITEM:
			return (ptrWeight(attr) == 1 && attrBF);
		default:
			return false;
	}
}
...
int Item :: acceSign(void)
{
	...
	{
		case CON_ITEM:
			if ( attr->type == FLOAT && val.i < 0 ) return 1;
...
bool Item :: acceFloat(void)
{
	attrib *ap = attr;
	switch ( type )
	{
		case CON_ITEM:
			return (ap->type == FLOAT);

		case ID_ITEM:
		case TEMP_ITEM:
		case ACC_ITEM:
			return (ap->type == FLOAT && !(ap->dimVect || ap->ptrVect));

		case DIR_ITEM:
		case INDIR_ITEM:
		case PID_ITEM:
			return (ap->type == FLOAT && !ap->dimVect && ptrWeight(ap) == 1);
...
Item *accItem(attrib *attr)
{
	...
	ip->attr = attr;
	if ( attr->type == FLOAT ) attr->isUnsigned = 0;
	return ip;
}
...
attrib *maxMonoAttr(Item *ip0, Item *ip1, int op)
{
	...
	if ( ip0->acceFloat() || ip1->acceFloat() )
		t = FLOAT;
	attrib *ap = newAttr(t);
	ap->isUnsigned = (us0 && us1 && t != FLOAT)? 1: 0;
	return ap;
}
...
bool same(Item *ip0, Item *ip1, bool fsr_check)
{
	...
	switch ( ip0->type )
	{
		...
		case LBL_ITEM:
		case ID_ITEM:
		case STR_ITEM:		return (strcmp(ip0->val.s, ip1->val.s) == 0);
