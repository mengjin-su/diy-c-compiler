...
void display(node *np, int level, FILE *file)
{
    ...
    switch ( np->type )
    {
        case NODE_CON:
            headMark(level, file);
            fprintf(file, "constant node: ");
            if ( np->con.attr->type == FLOAT )
            	printf("%f ", np->con.fvalue);
            else
               	printf("%ld ", np->con.value);
...
void display(attrib *ap, int level, char endln, FILE *file)
{
	...
	if ( ap )//&& ap->type )
	{
		...
		if ( ap->bitField )
			fprintf(file, " <%d:%d>", ap->bitField >> 8, ap->bitField & 0xff);
...
void display1(Pnode *pcode, FILE *file)
{
    ...
    switch ( pcode->type )
    {
		...
		case P_ARG_PASS:	fprintf(file, "P-CODE: PASS_ARG \t"); 	break;
...
//		case P_MOV_DEC:		fprintf(file, "P-CODE: = -- \t");		break;
		...
		case P_CALL:		fprintf(file, "P-CODE: P_CALL \t");		break;
		case P_MOV: 		fprintf(file, "P-CODE: P_MOV \t");		break;
		case AASM:			fprintf(file, "P-CODE: AASM \t");		break;
		case P_ASMFUNC:		fprintf(file, "P-CODE: ASM_FUNC \t");	break;
		case PRAGMA:		fprintf(file, "P-CODE: PRAGMA \t");		break;
...
void display(Item *ip, FILE *file)
{
	...
	switch ( ip->type )
	{
		...
		case CON_ITEM:	if ( ip->attr && ip->attr->type == FLOAT )
						{
							float *fp = (float*)&ip->val.i;
							fprintf(file, "%f", *fp);
						}
						else
							fprintf(file, "%d", ip->val.i);				break;
		case PID_ITEM:  fprintf(file, "[%s:%d]", ip->val.s, ip->bias);	break;
		case DIR_ITEM:  fprintf(file, "(0x%03x)", ip->val.i); 			break;
