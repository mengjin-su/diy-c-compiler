...
static char tempIndexMask[MAX_TEMP_COUNT];

static void tmpCollect(Pnode *head)
{
	memset(tempIndexMask, 0, MAX_TEMP_COUNT);
    while ( head && head->type != P_FUNC_END )
    {
        for (int i = 0; i < 3; i++)
        {
			...
			{
				if ( ip->val.i < MAX_TEMP_COUNT )
					tempIndexMask[ip->val.i] = 1;
			}
        }
        head = head->next;
    }
